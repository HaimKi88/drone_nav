...
class Backend(QObject):
    ...
    def __init__(self, ros_node: Node, parent=None):
        ...
        self._sub_location = self.node.create_subscription(NavSatFix, "/robot_location", self.cb_location, 10)
        self._pub_target = self.node.create_publisher(GeoPoint, "/target_location", 10)
...
def main():
    rclpy.init()
    node = rclpy.create_node("target_selection")
    ...
    # Start spinning ROS in background
    def spin_ros():
        while rclpy.ok():
